...
class SoftTemplate(Template):
    ...
    def __init__(self,
                 ...
                 classes = None,
                 initialize_from_classes: Optional[bool] = False,
                ):
        ...
        self.initialize_from_classes = initialize_from_classes
        self.text = text
    ...
    def generate_parameters(self) -> None:
        ...
        if self.initialize_from_vocab:
            print("Initializing from Vocab...")
            indicies = np.random.randint(4, 5004, self.num_tokens)
            soft_embeds = self.raw_embedding.weight[indicies].clone().detach()
        
        elif self.initialize_from_classes:
            print("Initializing from Classes...")
            classes_str = " ".join(self.classes)
            print(classes_str)
            tokenized_classes = self.tokenizer(classes_str)['input_ids']
            print(self.tokenizer.decode(tokenized_classes))
            multiplier = self.num_tokens/len(tokenized_classes)
            if multiplier < 1:
                indicies = tokenized_classes[:self.num_tokens]
            else:
                indicies = (tokenized_classes * int(multiplier+1))[:self.num_tokens]
            soft_embeds = self.raw_embedding.weight[indicies].clone().detach()
        else:
